...
class RecommendViewModel(private val stylishRepository: StylishRepository) : ViewModel(){
    ...
    fun getRecommendation(){
        viewModelScope.launch {
            try {
                val result = stylishRepository.getRecommendation(NativeLoginResult.nativeId.toString())
                when (result) {
                    is Result.Success -> {
    ...
    val sendUserTrackingFromRecommendationPageToProductDetailPage = fun(productId : Long){
        Log.i("Elven login", "RecommendationViewModel: sendUserTrackingFromRecommendationPageToProductDetailPage API is called")
        viewModelScope.launch {
            ...
            stylishRepository.sendUserTracking(uuid = ABTestUUID.UUID, eventType = "visit", actionFrom = "RecommendationPage", actionTo = "$productId", source = ABTestVersion.version )
            Log.i("Elven login", "RecommendationViewModel: sendUserTrackingFromRecommendationPageToProductDetailPage API finished")
        }
    }
